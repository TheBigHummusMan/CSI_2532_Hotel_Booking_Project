...
// registering for employe
/*
router.post("/register/employe", validInfo, async (req, res) => {
    try {
        const { nas, nom, password, email, ville, adressederue, codepostal } = req.body;

        // Check if user exists
        const user = await pool.query("SELECT * FROM employe WHERE employeeid = $1", [nas]);
        ...
        // Create new user
        const newUser = await pool.query(
            "INSERT INTO employe (employeeid, nom, addressid, password, email) VALUES($1, $2, $3, $4, $5) RETURNING *",
            [employeeid, nom, addressId, bcryptPassword, email]
        );
...
// login route for employe
router.post("/login/employe", validInfo,async(req,res)=>{
    try {
        const {email, password} = req.body

        const user = await pool.query("SELECT * FROM employe  WHERE email = $1",
            [email]
        );
